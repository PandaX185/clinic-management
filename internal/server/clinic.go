...
// ClinicResolver resolves a clinic ID to its schema slug. Implemented by
// clinic.Service via the store.
type ClinicResolver interface {
	// SlugForTenant validates the clinic exists and is active, returning
	// its slug (the only source of SQL schema names).
	SlugForTenant(ctx context.Context, clinicID uuid.UUID) (string, error)
}
...
func authClaimsFrom(c *gin.Context) *idsvc.Claims {
	v, ok := c.Get(httpctx.CtxClaims)
	if !ok {
		return nil
	}
	claims, _ := v.(*idsvc.Claims)
	return claims
}
...
func TenantMiddleware(resolver ClinicResolver, profiles ProfileResolver) gin.HandlerFunc {
	cache := newRoleCache()

	return func(c *gin.Context) {
		rawID := strings.TrimSpace(c.GetHeader(HeaderClinicID))
		tid, err := uuid.Parse(rawID)
		if err != nil {
			c.Error(apperr.Invalid("missing or invalid " + HeaderClinicID + " header"))
			c.Abort()
			return
		}
