...
// ListClinics returns all clinics that the current user has a profile in.
//
...
// @Produce json
// @Security BearerAuth
// @Success 200 {array} service.UserClinic
// @Router /auth/clinics [get]
func (h *Handler) ListClinics(c *gin.Context) {
	...
	clinics, err := h.svc.ListClinics(c.Request.Context(), userID)
	if err != nil {
		c.Error(err)
		return
	}
