...
type fakeClinicResolver struct{ slugs map[uuid.UUID]string }

func (f fakeClinicResolver) SlugForTenant(ctx context.Context, id uuid.UUID) (string, error) {
	s, ok := f.slugs[id]
	if !ok {
...
func setupRouter(resolver ClinicResolver, profiles ProfileResolver) (*gin.Engine, *httptest.ResponseRecorder) {
...
func doReq(r *gin.Engine, w *httptest.ResponseRecorder, tenantHeader string) {
	...
	if tenantHeader != "" {
		req.Header.Set(HeaderClinicID, tenantHeader)
	}
	r.ServeHTTP(w, req)
}
...
func TestTenantMiddleware_MissingHeader(t *testing.T) {
	resolver := fakeClinicResolver{slugs: map[uuid.UUID]string{}}
