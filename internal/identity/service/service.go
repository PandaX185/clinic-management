...
type Service struct {
	repo        Repository
	tokens      *TokenManager
	memberships ClinicMembershipProvider
	bcryptCost  int
}
...
type ClinicMembershipProvider interface {
	MembershipsForUser(ctx context.Context, userID uuid.UUID) ([]UserClinic, error)
}

// WithClinicMemberships wires a real membership resolver. It is required for
// ListClinics to return results; without it the call returns empty.
func (s *Service) WithClinicMemberships(p ClinicMembershipProvider) *Service {
	s.memberships = p
	return s
}

// ListClinics returns all clinics the user belongs to, with their role in
// each, via the injected membership provider (see wiring).
func (s *Service) ListClinics(ctx context.Context, userID uuid.UUID) ([]UserClinic, error) {
